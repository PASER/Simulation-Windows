...
std::string PASER_UU_RREP::detailedInfo() const {
    ...
    out << "Type : UURREP \n";
    out << "Querying node : " << srcAddress_var.S_addr.getIPAddress().str()
            << "\n";
    out << "Destination node : " << destAddress_var.S_addr.getIPAddress().str()
            << "\n";
    out << "Sequence : " << seq << "\n";
    out << "KeyNr : " << keyNr << "\n";
...
//        out << "route: " << temp.S_addr.getIPAddress().str() << "\n";
//    }
    ...
    for (std::list<address_list>::iterator it = temp.begin(); it != temp.end();
        ...
        out << "route: " << tempList.ipaddr.S_addr.getIPAddress().str() << "\n";
        ...
        for (std::list<address_range>::iterator it = temp2.begin();
            ...
            out << "range: " << tempRange.ipaddr.S_addr.getIPAddress().str()
                    << "\n";
        }
    }
...
//    len += sizeof(len);
    ...
    buf++;
    //Querying node
    memcpy(buf, srcAddress_var.S_addr.toString(10),
            sizeof(srcAddress_var.S_addr));
    buf += sizeof(srcAddress_var.S_addr);
    //Dest node
    memcpy(buf, destAddress_var.S_addr.toString(10),
    ...
    for (std::list<address_list>::iterator it = AddressRangeList.begin();
            it != AddressRangeList.end(); it++) {
        address_list temp = (address_list) *it;
        memcpy(buf, temp.ipaddr.S_addr.toString(10),
        ...
        for (std::list<address_range>::iterator it2 = temp.range.begin();
            ...
            struct in_addr temp_addr;
            temp_addr.S_addr = ((address_range) *it2).ipaddr.S_addr;
            memcpy(buf, temp_addr.S_addr.toString(10),
