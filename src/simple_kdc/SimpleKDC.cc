...
void SimpleKDC::handleMessage(cMessage *msg) {
    ...
    if (msg == resetMessage) {
        ...
        for (std::list<struct in_addr>::iterator it = nextHopList.begin();
            ...
            ev << "nextHopAddr: " << tempAddr.S_addr.getIPAddress().str()
                    << "\n";
...
//putchar('\n');
            free(data);
            socket.sendTo(message, tempAddr.S_addr.getIPAddress(),
    ...
    if (msg->isSelfMessage()) {
        if (dynamic_cast<crl_message *>(msg)) {
            ...
            socket.sendTo(message, message->getGwAddr().S_addr.getIPAddress(),
                    (int) par("port").longValue());
//            sendToUDP(message, (int)par("port").longValue(), message->getGwAddr().S_addr.getIPAddress(), (int)par("port").longValue());
        } else {
            delete msg;
        }
        return;
    ...
    if (dynamic_cast<crl_message *>(msg)) {
        ...
        ev << "message->getGwAddr(): "
                << message->getGwAddr().S_addr.getIPAddress().str() << "\n";
        struct in_addr src = message->getSrc();
        ev << "src: " << src.S_addr.getIPAddress().str() << "\n";
...
//        sendToUDP(message_to_send, PORT_for_CRL_out, message->getGwAddr().S_addr.getIPAddress(), PORT_for_CRL_out);
